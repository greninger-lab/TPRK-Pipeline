...
list.of.packages <- c(
  "ggplot2", "grid", "nplr", "plyr", "dplyr", "scales", "gridExtra", "RColorBrewer", "optparse", "cowplot",
...
if (ref_sample == FALSE && length(sample_names) > 1) {
  ...
  for (i in 1:(length(sample_names) - 1)) {
    ...
    h <- ggplot(sortedIllumina[which(sortedIllumina$Frequency > 0), ]) +
      geom_point(aes(y = Frequency, x = Sample, color = Read)) +
      geom_line(aes(y = Frequency, x = Sample, group = Read, color = Read)) +
