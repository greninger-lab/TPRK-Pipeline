...
list.of.packages <- c(
  "JuliaCall", "reticulate", "optparse", "ggplot2",
...
option_list <- list(
  make_option(c("-s", "--script_path"),
    type = "character", default = NULL, help = "Directory where scripts are located.",
    metavar = "character"
  ),
  ...
  make_option(c("-p", "--pacbio"),
    type = "character", default = FALSE, help = "Specify if these files are only PacBio.",
    metavar = "character", action = "store_true"
  ),
  make_option(c("-i", "--illumina"),
    type = "character", default = FALSE, help = "Specify if these files are only Illumina.",
    metavar = "character", action = "store_true"
  )
)
...
if (opt$illumina == FALSE) {
  ...
  for (count in c(1:length(filt))) {
    if (file.exists(filt[count])) {
      ...
      track <- dada2::fastqFilter(nop[count], filt[count],
        minLen = 1400, maxLen = 1800,
        maxN = 0,
