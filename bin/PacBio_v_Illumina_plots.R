...
option_list <- list(
  make_option(c("-p", "--path"),
    type = "character", default = NULL, help = "Path to allreads.csv",
    metavar = "character"
  ),
...
plot <- ggplot(alldata, aes(x = alldata[[illrf_col]], y = alldata[[pbrf_col]], color = Region)) +
  ...
  xlim(0, 100) +
  ylim(0, 100) +
  annotate("text", x = 85, y = 2, size = 3, label = anno, 3) +
  theme(legend.position = "bottom", legend.box = "horizontal") +
  theme(legend.key.width = unit(0.1, "cm")) +
  xlab(label = "Illumina Frequency") +
  ylab(label = "PacBio Frequency") +
...
plot_filt <- ggplot(alldatafilt, aes(x = alldatafilt[[illrf_col]], y = alldatafilt[[pbrf_col]], color = Region)) +
  geom_abline(
    slope = model_filt$coefficients[2], intercept = model_filt$coefficients[1], col = "slategray1",
    linetype = "solid"
  ) +
  geom_point(size = 1.5) +
  ggtitle(paste(sample_name, "Filtered")) +
  coord_equal() +
  theme(plot.title = element_text(size = 10, vjust = -2, hjust = 0.03)) +
  geom_abline(linetype = "dashed", color = "grey") +
  theme_classic() +
  ...
  xlim(0, 100) +
  ylim(0, 100) +
  annotate("text", x = 80, y = 2, size = 3, label = anno_filt, 3) +
...
figure <- grid.arrange(
  arrangeGrob(plot + theme(legend.position = "none"),
...
lim10plot <- ggplot(alldata, aes(x = alldata[[illrf_col]], y = alldata[[pbrf_col]], color = Region)) +
  xlim(0, 10) +
  ylim(0, 10) +
  geom_point(size = 2) +
  xlab(label = "Illumina Frequency") +
  ...
  annotate("text", x = 8, y = 0.5, label = anno, 3)
lim10plot_filt <- ggplot(alldatafilt, aes(x = alldatafilt[[illrf_col]], y = alldatafilt[[pbrf_col]], color = Region)) +
  xlim(0, 10) +
  ylim(0, 10) +
  geom_point(size = 2) +
  xlab(label = "Illumina Frequency") +
  ...
  ggtitle(paste(sample_name, "Filtered")) +
  scale_color_brewer(palette = "Set1") +
  annotate("text", x = 8, y = 0.5, label = anno_filt, 3)
figurelim10 <- grid.arrange(
  arrangeGrob(lim10plot + theme(legend.position = "none"),
    lim10plot_filt + theme(legend.position = "none"),
    nrow = 1
  ),
  mylegend,
  nrow = 2, heights = c(10, 1)
